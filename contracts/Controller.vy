...
def repay(_d_debt: uint256, _for: address = msg.sender, max_active_band: int256 = 2**255-1, use_eth: bool = True):
    ...
    if debt == 0:
        ...
        if xy[0] > 0:
            ...
            self.STABLECOIN.transferFrom(AMM.address, _for, xy[0])
        if xy[1] > 0:
...
def repay_extended(callbacker: address, callback_args: DynArray[uint256,5]):
    ...
    if total_stablecoins >= debt:
        ...
        if xy[0] > 0:
            self.STABLECOIN.transferFrom(AMM.address, self, xy[0])
...
def _liquidate(user: address, min_x: uint256, health_limit: uint256, frac: uint256, use_eth: bool,
    ...
    if min_amm_burn != 0:
        self.STABLECOIN.transferFrom(AMM.address, self, min_amm_burn)
    ...
    if debt > xy[0]:
        ...
        if xy[0] > debt:
            self.STABLECOIN.transferFrom(AMM.address, msg.sender, unsafe_sub(xy[0], debt))
...
def collect_fees() -> uint256:
    ...
    if borrowed_fees > 0:
        self.STABLECOIN.transferFrom(AMM.address, _to, borrowed_fees)
